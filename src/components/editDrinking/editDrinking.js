...
const EditDrinking=({navigation})=>{
  ...
  console.log('select drinking',drinking)
                ...
                Drinking.map((drinking, index) => {
                    return (
                        <>
                         ...
                         <Text key={index} style={{paddingTop:22,paddingLeft:8,fontSize:15,color:`${updateDrinking?.drinking===drinking?.drinking?'rgba(0, 150, 255, 1)':'black'}`}}>{drinking.drinking}</Text>
                         </TouchableOpacity>
                         {updateDrinking?.drinking===drinking?.drinking?  <Image source={rightTik} style={{width:12,height:12,marginTop:24,marginRight:30}} />:null}
                          </View>
                        </>
                    );
                })
