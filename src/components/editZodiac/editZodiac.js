...
const EditZodiac=({navigation})=>{
  ...
  console.log('select zodiac',zodiac)
...
return (
    <>
     <View style={{paddingTop:8,paddingLeft:8}}>
        {
                zodiacSign.map((zodiac, index) => {
                    return (
                        <>
                          <View style={{flexDirection:"row",justifyContent:'space-between'}}>
                          <TouchableOpacity onPress={()=>selectZodiacHandler(zodiac.zodiac)} >
                         <Text key={index} style={{paddingTop:22,paddingLeft:8,fontSize:15,color:`${updateZodiac?.zodiac===zodiac?.zodiac?'rgba(0, 150, 255, 1)':'black'}`}}>{zodiac.zodiac}</Text>
                         </TouchableOpacity>
                         {updateZodiac?.zodiac===zodiac?.zodiac?  <Image source={rightTik} style={{width:12,height:12,marginTop:24,marginRight:30}} />:null}
                          </View>
                        </>
                    );
                })
