...
const EditEating=({navigation})=>{
  ...
  console.log('select eating',eating)
...
return (
    <>
     <View style={{paddingTop:8,paddingLeft:8}}>
        {
                Eating.map((eating, index) => {
                    return (
                        <>
                          <View style={{flexDirection:"row",justifyContent:'space-between'}}>
                          <TouchableOpacity onPress={()=>selectEatingHandler(eating.eating)} >
                         <Text key={index} style={{paddingTop:22,paddingLeft:8,fontSize:15,color:`${updateEating?.eating===eating?.eating?'rgba(0, 150, 255, 1)':'black'}`}}>{eating.eating}</Text>
                         </TouchableOpacity>
                         {updateEating?.eating===eating?.eating?  <Image source={rightTik} style={{width:12,height:12,marginTop:24,marginRight:30}} />:null}
                          </View>
                        </>
                    );
                })
